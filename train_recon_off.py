...
from data_provider.data_loader_emb_recon import Dataset_ETT_hour, Dataset_ETT_minute, Dataset_Custom, Dataset_PEMS
from model.TimeKD_recon_off import Dual
...
class trainer:
    ...
    def train(self, embeddings, real):
        self.model.train()
        self.optimizer.zero_grad()
        predict= self.model(embeddings)
    ...
    def eval(self, embeddings, real_val):
        self.model.eval()
        with torch.no_grad():
            predict = self.model(embeddings)
...
def main():
    ...
    for i in range(1, args.epochs + 1):
        ...
        for iter, (x, y, embeddings) in enumerate(train_loader):
            ...
            metrics = engine.train(train_embedding, trainy)
        ...
        for iter, (x, y, embeddings) in enumerate(val_loader):
            ...
            metrics = engine.eval(val_embedding, valy)
        ...
        if mvalid_loss < loss:
            ...
            if i <= 10:
                ...
                for iter, (x, y, embeddings) in enumerate(test_loader):
                    testy = torch.Tensor(y).to(device)
                    test_embedding = torch.Tensor(embeddings).to(device)
                    with torch.no_grad():
                        preds = engine.model(test_embedding)
