...
class MSK(nn.Module):
    ...
    def custom_forward(self,
        ...
        for i, (block, layer_past) in enumerate(zip(self.gpt2.h, past_key_values)):
            ...
            attention_mask = adjacency_matrix
...
class GenPromptEmb(nn.Module):
    ...
    def _prepare_prompt(self, HD_GT, HD, x, y, x_mark, y_mark, i, j):
        ...
        values_str = ", ".join([f"{value:.2f}" if not value.is_integer() else f"{int(value)}" for value in values])
        # values_str = ", ".join([f"{value:g}" if not value.is_integer() else f"{int(value)}" for value in values])


        gt_values = y[i, :, j].flatten().tolist()
        # gt_str = ", ".join([str(int(value)) for value in gt_values])
        gt_str = ", ".join([f"{value:.2f}" if not value.is_integer() else f"{int(value)}" for value in gt_values])
        # gt_str = ", ".join([f"{value:g}" if not value.is_integer() else f"{int(value)}" for value in gt_values])
    ...
    def forward(self, HD_GT_token, HD_token, token_types):
        ...
        HD_GT_emb = self.gpt2(HD_GT_token, adjacency_matrix=mask_adj_GT)
        HD_emb = self.gpt2(HD_token, adjacency_matrix=mask_adj_HD)
    ...
    def generate_embeddings(self, x, y, x_mark, y_mark):
        ...
        sub_out = self.sub_ac(prompt_emb_HD_1, prompt_emb_GT_1, prompt_emb_GT_1)
